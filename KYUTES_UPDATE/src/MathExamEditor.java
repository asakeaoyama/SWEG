	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==addButt) {
			...
			try {
				...
				String insCreateExamPaper = "create table `"+EA[0]+"Paper` (`quetion` varchar(100) primary key, `ans` varchar(100));";
				/*
				st.executeUpdate(insCreateExamPaper);
				ResultSet rs = st.executeQuery("Select * from `mathQ`");
				for(int i =0;i<5;i++) {
					String insFillExamPaper = "insert into `"+EA[0]+"Paper` 
				}
				*/
			} catch (Exception e1) {
				System.out.println(e1);
